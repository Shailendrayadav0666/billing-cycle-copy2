...
# output, --dangerously-skip-permissions because this runs unattended on an ephemeral CI runner with
# no human to answer permission prompts, --output-format text for a plain-text response.
printf '%s' "$BRIEF" | claude -p --dangerously-skip-permissions --output-format text \
  || report_and_exit "the repair CLI invocation failed on attempt ${attempt}." 1
