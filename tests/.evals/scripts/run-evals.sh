...
${diff}"
  ...
  # Flags resolved from `claude --help` at generation time (v2.1.259): -p/--print for non-interactive
  # output, --dangerously-skip-permissions (unattended CI runner, no human to answer prompts),
  # --output-format text for a plain-text response the JSON extractor below then parses.
  resp="$(printf '%s' "$prompt" | claude -p --dangerously-skip-permissions --output-format text 2>"$claude_err_file")"; rc=$?
  ...
  if [ -z "$json" ]; then
    resp="$(printf '%s' "$prompt" | claude -p --dangerously-skip-permissions --output-format text 2>"$claude_err_file")" || true
    json="$(printf '%s' "$resp" | sed -n '/{/,/}/p' | jq -c '.' 2>/dev/null || true)"
  fi
  rm -f "$claude_err_file"
  if [ -z "$json" ]; then
