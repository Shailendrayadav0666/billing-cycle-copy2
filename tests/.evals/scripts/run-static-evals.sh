...
# Stack: Python (src/backend). D1/D2/D4/D5/D6 have no native baseline flag, so each is run through
# delta_diff(), which captures the same command at BASE_SHA and HEAD and counts only NEW tuples.
if command -v ruff >/dev/null 2>&1; then
  delta_diff D1_lint \
    "ruff check --output-format=json src/backend 2>/dev/null | jq -r '.[] | [.code, .filename, .message] | @tsv' | sort -u"
else
  record D1_lint ERROR "ruff not installed"
fi

if command -v mypy >/dev/null 2>&1; then
  delta_diff D2_types \
    "mypy --no-error-summary --ignore-missing-imports src/backend 2>/dev/null | grep -E ': error:' | awk -F': error:' '{print \"mypy-error\t\" \$1 \"\t\" \$2}' | sort -u"
else
  record D2_types ERROR "mypy not installed"
fi

if command -v pip-audit >/dev/null 2>&1; then
  delta_diff D4_deps \
    "pip-audit -r src/backend/requirements.txt -f json 2>/dev/null | jq -r '.dependencies[]? // empty | . as \$d | (\$d.vulns // [])[] | [.id, \"src/backend/requirements.txt\", .description] | @tsv' | sort -u"
else
  record D4_deps ERROR "pip-audit not installed"
fi

if command -v pip-licenses >/dev/null 2>&1; then
  DISALLOWED_JSON="$(jq -c '.thresholds.disallowedLicenses // []' "$CONFIG")"
  delta_diff D5_licenses \
    "pip-licenses --format=json 2>/dev/null | jq -r --argjson d '${DISALLOWED_JSON}' '.[] | select(.License as \$l | \$d | index(\$l)) | [.License, .Name, .Version] | @tsv' | sort -u"
else
  record D5_licenses ERROR "pip-licenses not installed"
fi

if command -v radon >/dev/null 2>&1; then
  MAXCC="$(jq -r '.thresholds.maxCyclomaticComplexity // 12' "$CONFIG")"
  delta_diff D6_complexity \
    "radon cc src/backend -j 2>/dev/null | jq -r --argjson m '${MAXCC}' 'to_entries[] | .key as \$f | .value[] | select(.complexity > \$m) | [.name, \$f, (\"complexity \" + (.complexity|tostring))] | @tsv' | sort -u"
else
  record D6_complexity ERROR "radon not installed"
fi
