...
run_features() { # $@ = feature files
  ...
  # Python / pytest-bdd. Each spec/behavior/<slug>.feature has a matching
  # tests/behavior/test_<slug-with-underscores>.py that binds it via pytest_bdd.scenarios().
  # The cycle-level spec/behavior.feature maps to tests/behavior/test_cross_story.py.
  local modules=() f base mod
  for f in "$@"; do
    if [ "$f" = "spec/behavior.feature" ]; then
      mod="tests/behavior/test_cross_story.py"
    else
      base="$(basename "$f" .feature)"
      mod="tests/behavior/test_${base//-/_}.py"
    fi
    if [ ! -f "$mod" ]; then
      echo "run.sh: no pytest-bdd module for feature '$f' (expected $mod)" >&2
      return 2
    fi
    modules+=("$mod")
  done
  python3 -m pytest -q "${modules[@]}"
